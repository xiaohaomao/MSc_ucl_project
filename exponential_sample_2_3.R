...
d1=read.csv('C:/Users/xiaohao/Documents/Rwork/TestData_29_08_01.CSV', header=TRUE)
...
name_of_excel<-list("2908_test_exponential_output_01.xlsx")
name_of_plot<-list("2908_test_exponential_output_01")
...
for (i in 1:1){
  ...
  for(k in 2:2){
    ...
    plot(m2,legend.pos = c(100,1),xlab="time",ylab = "survival of probability")
...
Error_vary<-abs(percentage_of_expected-percentage_of_observed)/percentage_of_observed

for (i in 1:42){
  for (j in 1:5){
    if (Error_vary[i,j]<=10^100)
    {
      cat("ith",i,"jth",j,"\n")
    }else{
      Error_vary[i,j]<-NA
...
Error_vary_final<-colMeans(Error_vary,na.rm=TRUE)
