...
d1=read.csv('C:/Users/xiaohao/Documents/Rwork/TestData_24_08_01.CSV', header=TRUE)
d1$age<-d1$age-45 # transform ages
v1=read.csv('C:/Users/xiaohao/Documents/Rwork/vector_test_24_08_000.CSV', header=TRUE)
...
name_of_excel<-list("1208_test_fixed_output_07.xlsx")
name_of_plot<-list("1208_test_fixed_output_07")

v2=v1$length_vector

time_interval<-c(5,10)

mse_o_e<-matrix(NA,ncol=5,nrow=6)
ER_o_e<-matrix(NA,ncol=5,nrow=6)


for (m in 3:3){
  
  
  cat("the mth timeinterval",m,"\n")
  ...
  for (i in 1:1){
    ...
    for(k in 2:2){
      ...
      options(digits=3)
      prevalence<-prevalence.msm(m2,times=seq(1,50,1))
      ...
      plot.prevalence.msm(m2,mintime = 0,maxtime=50)
